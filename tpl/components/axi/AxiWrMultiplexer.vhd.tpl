...
architecture AxiWrMultiplexer of AxiWrMultiplexer is
  ...
  type t_NativeAxiW_do is record
    ready   : {{x_pm_axiWr.type.x_tlogic}};
  end record;
...
begin
  ...
  begin
    ...
    po_axiWr_ms.awaddr   <= so_masterA_od.addr;
    po_axiWr_ms.awlen    <= so_masterA_od.len;
    po_axiWr_ms.awsize   <= so_masterA_od.size;
    po_axiWr_ms.awburst  <= so_masterA_od.burst;
    po_axiWr_ms.awlock   <= so_masterA_od.lock;
    po_axiWr_ms.awcache  <= so_masterA_od.cache;
    po_axiWr_ms.awprot   <= so_masterA_od.prot;
    po_axiWr_ms.awqos    <= so_masterA_od.qos;
    po_axiWr_ms.awregion <= so_masterA_od.region;
    po_axiWr_ms.awid     <= so_masterA_od.id;
    po_axiWr_ms.awuser   <= so_masterA_od.user;
    po_axiWr_ms.awvalid  <= so_masterA_od.valid;
    po_axiWr_ms.wdata    <= so_masterW_od.data;
    po_axiWr_ms.wstrb    <= so_masterW_od.strb;
    po_axiWr_ms.wlast    <= so_masterW_od.last;
    po_axiWr_ms.wuser    <= so_masterW_od.user;
    po_axiWr_ms.wvalid   <= so_masterW_od.valid;
    po_axiWr_ms.bready   <= so_masterB_do.ready;

    -- si_masterA_do <= f_nativeAxiWrSplitA_sm(pi_axiWr_sm);
    si_masterA_do.ready <= pi_axiWr_sm.awready;

    -- si_masterW_do <= f_nativeAxiWrSplitW_sm(pi_axiWr_sm);
    si_masterW_do.ready <= pi_axiWr_sm.wready;

    -- si_masterB_od <= f_nativeAxiWrSplitB_sm(pi_axiWr_sm);
    si_masterB_od.resp    <= pi_axiWr_sm.bresp;
    si_masterB_od.id      <= pi_axiWr_sm.bid;
    si_masterB_od.user    <= pi_axiWr_sm.buser;
    si_masterB_od.valid   <= pi_axiWr_sm.bvalid;
    ...
    for v_idx in 0 to g_PortCount-1 loop
      ...
      po_axiWrs_sm(v_idx).awready  <= so_slavesA_do(v_idx).ready;
      po_axiWrs_sm(v_idx).wready   <= so_slavesW_do(v_idx).ready;
      po_axiWrs_sm(v_idx).bresp    <= so_slavesB_od(v_idx).resp;
      po_axiWrs_sm(v_idx).bid      <= so_slavesB_od(v_idx).id;
      po_axiWrs_sm(v_idx).buser    <= so_slavesB_od(v_idx).user;
      po_axiWrs_sm(v_idx).bvalid   <= so_slavesB_od(v_idx).valid;

      -- si_slavesA_od(v_idx) <= f_nativeAxiWrSplitA_ms(pi_axiWrs_ms(v_idx));
      si_slavesA_od(v_idx).addr   <= pi_axiWrs_ms(v_idx).awaddr;
      si_slavesA_od(v_idx).len    <= pi_axiWrs_ms(v_idx).awlen;
      si_slavesA_od(v_idx).size   <= pi_axiWrs_ms(v_idx).awsize;
      si_slavesA_od(v_idx).burst  <= pi_axiWrs_ms(v_idx).awburst;
      si_slavesA_od(v_idx).lock   <= pi_axiWrs_ms(v_idx).awlock;
      si_slavesA_od(v_idx).cache  <= pi_axiWrs_ms(v_idx).awcache;
      si_slavesA_od(v_idx).prot   <= pi_axiWrs_ms(v_idx).awprot;
      si_slavesA_od(v_idx).qos    <= pi_axiWrs_ms(v_idx).awqos;
      si_slavesA_od(v_idx).region <= pi_axiWrs_ms(v_idx).awregion;
      si_slavesA_od(v_idx).id     <= pi_axiWrs_ms(v_idx).awid;
      si_slavesA_od(v_idx).user   <= pi_axiWrs_ms(v_idx).awuser;
      si_slavesA_od(v_idx).valid  <= pi_axiWrs_ms(v_idx).awvalid;

      -- si_slavesW_od(v_idx) <= f_nativeAxiWrSplitW_ms(pi_axiWrs_ms(v_idx));
      si_slavesW_od(v_idx).data     <= pi_axiWrs_ms(v_idx).wdata;
      si_slavesW_od(v_idx).strb     <= pi_axiWrs_ms(v_idx).wstrb;
      si_slavesW_od(v_idx).last     <= pi_axiWrs_ms(v_idx).wlast;
      si_slavesW_od(v_idx).user     <= pi_axiWrs_ms(v_idx).wuser;
      si_slavesW_od(v_idx).valid    <= pi_axiWrs_ms(v_idx).wvalid;

      -- si_slavesB_do(v_idx) <= f_nativeAxiWrSplitB_ms(pi_axiWrs_ms(v_idx));
      si_slavesB_do(v_idx).ready   <= pi_axiWrs_ms(v_idx).bready;
    end loop;
  end process;
